...
import com.papun.service.ViewAssosiateDateService;

@WebServlet(urlPatterns={"/viewAssosiateDateWise"})
public class viewAssosiateDateWise extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    
    public viewAssosiateDateWise() {
        super();
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if( page_type.equals("viewAssosiateDateWise")) {
			String page=(String) request.getParameter("page");
	    	
	    	System.out.println("not succefully login");
			request.setAttribute("StatusCode","200");
			request.setAttribute("StatusMessage","OK");
			request.setAttribute("ErrorMessage",new ViewAssosiateDateService().getResponseHtml(id,page,session));
			//request.getRequestDispatcher("index.jsp").forward(request, response); 
			request.getRequestDispatcher("/viewAssosiateDateWise.jsp").forward(request, response); 
